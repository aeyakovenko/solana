...
impl ReplayStage {
    ...
    fn entries_to_blocks(entries: Vec<Entry>) -> Vec<(Vec<Entry>, u64, u64)> {
        let mut blocks = vec![];
        for e in entries {
            let current = e.tick_height / BLOCK_TICK_COUNT;
            let prev = current - 1;
            if blocks.is_empty() {
    ...
    fn process_entries(
        ...
        let _start_ticks = bank.tick_height() / BLOCK_TICK_COUNT;
        ...
        for (entries, current_slot, base_slot) in blocks {
            ...
            assert_eq!(
                ...
                0,
                "expecting the first entry in a block to be a tick"
            );
        ...
        let now = Instant::now();
        let _end_ticks = bank.tick_height() / BLOCK_TICK_COUNT;
